...
public:
    ...
    float z_near=0.01;
    float z_far=0.28347224+0.261034;//
...
*/
    ...
    void set_vertex_color_buffer(PiecewiseStrands hair, GLuint &vertexbuffer, GLuint &colorbuffer,  glm::mat4 MVP, Vec3d camera_pos, bool is_render_head=true,bool only_render_head=false) {
        ...
        if(is_render_head or only_render_head)
...
//rotation is xzy translation is xyz
    ...
    glm::mat4 set_camera(GLuint program_id, glm::vec3 rotation_head=glm::vec3(0,0,0), glm::vec3 translation_head=glm::vec3(0,0,0))
    {
        ...
        Projection = glm::ortho(-0.36300416f,0.36300416f,-0.36300416f,0.36300416f,0.0f,100.0f);//for neural hd

        // Camera matrix
        glm::vec3 eye_pos = glm::vec3(0.0,1.58652416,0.28347224);
        ...
        glm::mat4 View = glm::lookAt(
                ...
                eye_pos + look_pos, // and looks at the origin
        ...
        // glm::mat4 m1 = View * Model;
        // string out_s="";
        // out_s=out_s+glm::to_string(Projection)+"\n"+glm::to_string(View)+"\n"+glm::to_string(Model);//+"\n"+glm::to_string(mvp)+"\n";
        // std::cout<<out_s<<std::endl;
    ...
    glm::mat4 get_model_matrix(glm::vec3 rotation_head=glm::vec3(0,0,0), glm::vec3 translation_head=glm::vec3(0,0,0))
    {
        ...
        glm::mat4 Model = glm::translate(translation_head+glm::vec3(-0.00703544,1.58652416,0.01121912)) * glm::orientate4(rotation_head) * glm::translate(glm::vec3(0.00703544,-1.58652416,-0.01121912));
        return Model;
    }
    ...
    void savecurrentDepthImg(string filename, int width, int height)
    {
        ...
        cv::Mat depth_exr(height, width,CV_8UC3, cv::Scalar(0,0,0));

        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                float d = depth[j+i*width];
                float z_eye = (d * (z_far-z_near) + z_near)/(z_far-z_near);
                int d1 = int(d*255);
                // float z = 2.0 *z_near * z_far / (z_far + z_near - (d*2-1.0) * (z_far - z_near));
                // d=1-(z-0.5);
                // d=d*1.5;
                depth_exr.at<cv::Vec3b>(i, j)[0] =d1;
                depth_exr.at<cv::Vec3b>(i, j)[1] = d1;
                depth_exr.at<cv::Vec3b>(i, j)[2] = d1;
            }
        }

        cv::imwrite(filename+".png", (depth_exr));
    ...
    void savecurrentImg(string filename, int width, int height)
    {
        ...
        // cv::blur(renderImg_exr, renderImgBlur_exr,cv::Size(3,3));
        ...
        cv::flip(orientImg_exr, orientImgFlip_exr,0);//上下翻转
        ...
        cv::cvtColor(orientImg_png, orientImg_png, cv::COLOR_RGB2BGR);//for neural hd
