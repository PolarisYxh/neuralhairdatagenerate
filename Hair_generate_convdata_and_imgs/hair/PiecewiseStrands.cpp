...
bool PiecewiseStrands::deSerializeFloat_no_visibility_seg_vec(FILE *f)
{
	...
	int num,point_num;
	fread(&num, sizeof(int), 1, f);
	fread(&point_num, sizeof(int), 1, f);
	cout << "Strand number: " << num << "\n";
	cout << "Point number: " << point_num << "\n";
	//int point_num = 0;
	//int length = 0;
	...
	for (int i = 0; i < num; i++)
	{
		...
		bool s= strand.deSerializeFloat_no_visibility_seg_vec(f,segments[i]);
        if(s==false)
        {
            cout << "Failed to read models from binary file. Wrong strand point number" << endl;
            return false;
...
bool PiecewiseStrands::deSerializeFloat_no_visibility_seg_vec(const char* fileName)
{
	FILE *f = fopen(fileName, "rb");
	bool result= deSerializeFloat_no_visibility_seg_vec(f);
	fclose(f);
	return result;
}
